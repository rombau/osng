...
gulp.task('fonts', function () {
	return gulp.src(config.globs.fonts).pipe(gulp.dest(path.join('mobile', 'lib', 'fonts')));
});
...
let
CSS_TEMP_DIR = temp.path({
	prefix : 'maui-css'
});

gulp.task('css:less', function () {
	gulp.src(['src/less/mobile-angular-ui-base.less','src/less/mobile-angular-ui-desktop.less','src/less/sm-grid.less']).pipe(less({
...
gulp.task('css:less:wait', waitFor([path.join(CSS_TEMP_DIR, 'sm-grid.css'),
...
gulp.task('css:concat', function () {
	return gulp.src([path.join(CSS_TEMP_DIR, 'sm-grid.css'),path.join(CSS_TEMP_DIR, 'mobile-angular-ui-base.css')]).pipe(concat('mobile-angular-ui-base.css')).pipe(
		gulp.dest(path.join('mobile', 'lib', 'css')));
});

gulp.task('css:copy', function () {
	return gulp.src([path.join(CSS_TEMP_DIR, 'mobile-angular-ui-hover.css'),path.join(CSS_TEMP_DIR, 'mobile-angular-ui-desktop.css')]).pipe(gulp.dest(path.join('mobile', 'lib', 'css')));
});

gulp.task('css:minify', function () {
	return gulp.src(path.join('mobile', 'lib', 'css', '*.css')).pipe(csso()).pipe(rename({
		suffix : '.min'
	})).pipe(gulp.dest(path.join('mobile', 'lib', 'css')));
});
...
gulp.task('css', function (done) {
	seq('css:rmtemp', 'css:less', 'css:less:wait', 'css:concat', 'css:copy', 'css:minify', 'css:rmtemp', done);
});
...
let
compileJs = function (dest, src) {
	return gulp.src(src).pipe(sourcemaps.init()).pipe(concat(dest)).pipe(gulp.dest(path.join('dist', 'js'))).pipe(uglify()).pipe(rename({
		suffix : '.min'
	})).pipe(sourcemaps.write('.')).pipe(gulp.dest(path.join('dist', 'js')));
};
...
gulp.task('build:wait', waitFor(['dist/css/mobile-angular-ui-base.css',
...
/*
 * ============================================================== = Setup live reloading on source
 * changes = ==============================================================
 */

gulp.task('livereload:demo', function () {
	return gulp.src(config.globs.livereloadDemo).pipe(connect.reload());
});
